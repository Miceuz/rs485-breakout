 ...
 * - I2C_MAXWAIT = 0..32767 number of retries in i2c_start_wait. 0 means never
 stop.
 */
 ...
 * - added "internal pullups", but be careful since the option stretches the I2C
 * specs Version 1.3:
 * - added "__attribute__ ((used))" for all functions declared with
 * "__attribute__ ((noinline))" Now the module is also usable in Arduino 1.6.11+
 * Version 1.2:
...
// Similar to start function, but wait for an ACK! Will timeout if I2C_MAXWAIT >
// 0.
bool __attribute__((noinline)) i2c_start_wait(uint8_t addr)
    __attribute__((used));
...
i2c_wait_scl_high(void) asm("ass_i2c_wait_scl_high") __attribute__((used));
...
#else
{
  __asm__ __volatile__(
      ...
      : [ SCLDDR ] "I"(SCL_DDR), [ SCLPIN ] "I"(SCL_PIN), [ SCLIN ] "I"(SCL_IN),
        [ SCLOUT ] "I"(SCL_OUT), [ SDADDR ] "I"(SDA_DDR),
        [ SDAPIN ] "I"(SDA_PIN), [ SDAIN ] "I"(SDA_IN),
        [ SDAOUT ] "I"(SDA_OUT));
  return true;
}
#endif
...
#else
{
  __asm__ __volatile__(
      ...
      : [ SDADDR ] "I"(SDA_DDR), [ SDAPIN ] "I"(SDA_PIN),
        [ SDAOUT ] "I"(SDA_OUT), [ SCLOUT ] "I"(SCL_OUT), [ SCLIN ] "I"(SCL_IN),
        [ SCLPIN ] "I"(SCL_PIN));
  ...
  __asm__ __volatile__
      ...
      (
...
#endif
          " rcall 	ass_i2c_delay_half	;delay  T/2 \n\t"
          " sbis     %[SCLIN],%[SCLPIN]      ;check for clock stretching "
          "slave\n\t"
          " rcall    ass_i2c_wait_scl_high   ;wait until SCL=H\n\t"
#if I2C_PULLUP
          ...
          : [ SCLDDR ] "I"(SCL_DDR), [ SCLPIN ] "I"(SCL_PIN),
            ...
            [ SDAOUT ] "I"(SDA_OUT), [ SDADDR ] "I"(SDA_DDR),
            [ SDAPIN ] "I"(SDA_PIN));
...
#else
{
  __asm__ __volatile__(
      ...
      " breq       _Li2c_start_wait_done  ;if zero reached, exit with false -> "
      "r24 already zero!\n\t"
#endif
      ...
      :
      : [ SDADDR ] "I"(SDA_DDR), [ SDAPIN ] "I"(SDA_PIN),
        [ SDAOUT ] "I"(SDA_OUT), [ SCLIN ] "I"(SCL_IN), [ SCLPIN ] "I"(SCL_PIN),
...
#else
{
  __asm__ __volatile__(
      ...
      " ret \n\t"
      "              ;; + 4 = 17C + 2X for acknowldge bit" ::[SCLDDR] "I"(
          SCL_DDR),
      [ SCLPIN ] "I"(SCL_PIN), [ SCLIN ] "I"(SCL_IN), [ SDAOUT ] "I"(SDA_OUT),
      [ SCLOUT ] "I"(SCL_OUT), [ SDADDR ] "I"(SDA_DDR), [ SDAPIN ] "I"(SDA_PIN),
      [ SDAIN ] "I"(SDA_IN));
