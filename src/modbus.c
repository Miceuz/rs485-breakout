 ...
 *
 *  	Returns 1 if the crc check is positive, returns 0 and saves the
 *calculated CRC bytes at the end of the data array if it fails.
 *
 */
uint8_t crc16(volatile uint8_t *ptrToArray,
              uint8_t inputSize) // A standard CRC algorithm
{
  uint16_t out = 0xffff;
  uint16_t carry;
  unsigned char n;
  ...
  // out=0x1234;
  if ((ptrToArray[inputSize] == out % 256) &&
      (ptrToArray[inputSize + 1] == out / 256)) // check
  {
    return 1;
  } else {
...
static inline void stop15Timer() { TIMSK2 &= ~_BV(OCIE2A); }

static inline bool isTimer15Off() { return 0 == (TIMSK2 & _BV(OCIE2A)); }
...
void modbusSetAddress(uint8_t address) { ownAddress = address; }
...
/* @brief:  Returns the amount of requested data objects (coils, discretes,
 * registers)
 *
 */
...
/* @brief: Returns the address of the first requested data object (coils,
 * discretes, registers)
 *
 */
...
bool isBroadcastFrame() { return ADDRESS_BROADCAST == rxbuffer[0]; }
...
bool validateFrame() {
  ...
  if (isFunction(fcReadHoldingRegisters) || isFunction(fcReadInputRegisters)) {
    ...
    // requested amount of bytes must fit into buffer together with address,
    // function code, and 2 bytes of crc
    ...
    if (rxbuffer[6] <
        requestedAmount * 2) { // enough data declared in the frame?
    ...
    if ((dataPos - 6) < 2) { // enough data received? should be exactly 2 bytes,
                             // but we allow more, will use just the first two
      modbusSendException(ecIllegalDataValue);
      return false;
    }
    return true;
...
/**
 * @brief	Character reception interrupt. Code in this interrupt handling
 * routine plus two subsequent timer interrupt routines implement a RTU
 * transmission mode state diagram as per "MODBUS over serial line specification
 * and implementation guide V1.02" found at www.modbus.org
 */
...
bool modbusIsTXComplete() { return txComplete; }
...
void modbusInit(uint8_t baudIdx, uint8_t parityIdx) {
  ...
  UART_CONTROL = (1 << RXCIE) | (1 << RXEN) |
                 (1 << TXEN); // USART receiver and transmitter and receive
                              // complete interrupt
  UART_STATUS |= (1 << U2X);  // double speed mode.
  UCSRC = (3 << UCSZ0) | (parityModes[parityIdx] << UPM00); // Frame Size

  if (0 == parityIdx) {
    UCSRC |= _BV(USBS0); // if there is no parity, use two stop bits as per
                         // Modbus over serial spec v1.02
  }
...
bool modbusIsFrameAvailable() { return frameAvailable; }

uint8_t modbusGetRegisterNumber(void) { return rxbuffer[3]; }

uint8_t modbusGetFunctionCode(void) { return rxbuffer[1]; }
...
void modbusSendException(unsigned char exceptionCode) {
  rxbuffer[1] |=
      (1 << 7); // setting MSB of the function code (the exception flag)
  rxbuffer[2] =
      exceptionCode; // Exceptioncode. Also the last byte containing data
  modbusSendMessage(2);
}

/* @brief: copies a single or multiple bytes from one array of bytes to an array
 * of 16-bit-words
 *
 */
void intToModbusRegister(volatile uint16_t *inreg, volatile uint8_t *outreg,
                         uint8_t amount) {
...
/* @brief: copies a single or multiple 16-bit-words from one array of integers
 * to an array of bytes
 *
 */
void modbusRegisterToInt(volatile uint8_t *inreg, volatile uint16_t *outreg,
                         uint8_t amount) {
  for (uint8_t c = 0; c < amount; c++) {
...
/* @brief: Handles single/multiple register reading and single/multiple register
 * writing.
 *
 *         Arguments: - ptrToInArray: pointer to the user's data array
 * containing registers
 *                    - startAddress: address of the first register in the
 * supplied array
 *                    - size: input array size in the requested format
 * (16bit-registers)
 *
 */
uint8_t modbusExchangeRegisters(volatile uint16_t *ptrToInArray,
                                uint16_t size) {
  uint8_t requestedAmount = getRequestedRegisterCount();
  uint8_t requestedAdr = getRequestedAddress();

  if ((requestedAmount + requestedAdr) <= size) {
    if (isFunction(fcReadHoldingRegisters) ||
        isFunction(fcReadInputRegisters)) {
      rxbuffer[2] = (uint8_t)(requestedAmount * 2); // payload size
      intToModbusRegister(ptrToInArray + requestedAdr, rxbuffer + 3,
                          rxbuffer[2]);
      modbusSendMessage(2 + rxbuffer[2]);
      return 1;
    } else if (isFunction(fcPresetMultipleRegisters)) {
      modbusRegisterToInt(rxbuffer + 7, ptrToInArray + requestedAdr,
                          requestedAmount);
      modbusSendMessage(5);
      return 1;
    } else if (isFunction(fcPresetSingleRegister)) {
    ...
    // modbusSendException(ecSlaveDeviceFailure); //inapropriate call of
...
bool modbusIsIdle() { return STATE_IDLE == busState; }